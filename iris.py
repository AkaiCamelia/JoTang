...
input, x_test, label, y_test = train_test_split( dataset['data'], dataset['target'], test_size=0.20, shuffle=True )
...
#softmax函数的作用是将输入值转变为（0，1）区间的数据，让其变成一个总和为1的概率分布，有利于后续的分类
class NET(nn.Module):
...
#监督学习，因为有标签作为目标。
#batch 未明确声明，为默认大小，即整个数据集。
#像本题取整体数据及作为batch size并不适用于大数据集。因为这样会花费很多内存，同时计算速度慢。
#大数据集应将整体数据集划分为若干个合适大小的minibatch